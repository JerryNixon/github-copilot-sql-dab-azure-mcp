// ── App UI Layer ──
// Depends on: auth.js (initAuth, currentAccount, handleLogin, handleLogout)
//             dab.js (fetchTodos, createTodo, toggleTodo, deleteTodo)
...
function renderTasks() {
    ...
    form.style.display = currentAccount ? 'flex' : 'none';
    ...
    if (tasks.length === 0) {
        list.innerHTML = `
            <div class="empty-state">
                ...
                <p>${currentAccount ? 'No tasks yet — add one above' : 'Sign in to see your tasks'}</p>
            </div>`;
        return;
    }
